...
La boucle
Observer — lire l'état réel avant d'agir : gh pr checks, logs CI, diff courant, fichiers concernés. Ne jamais supposer l'état ; le vérifier.
Décider — une action ciblée, pas une réécriture large. Préférer le fix minimal qui adresse la cause identifiée dans les logs.
Exécuter — appliquer l'action (edit, commit).
Vérifier — contrôler contre le critère d'arrêt (voir ci-dessous), pas contre une impression.
Décider la suite — continuer, corriger différemment, rollback, ou stop.
Stop — dès que le critère vérifiable est atteint. Ne pas continuer "pour être sûr".
