...
description: Orchestration d'un dialogue structuré entre deux LLMs (A et B) autour d'un même
  ...
  convergence définie, compromis d'indépendance assumé, séquencement anti-court-circuit et
  filtre de pertinence des acquis), calibration à trois paliers (avec protection
  anti-ancrage), et juge comme générateur d'angles morts.
version: 2.3
language: fr
---

# dual-llm-contradiction-dialog (v2.3)
...
>   « convergence ».
> - [v2.2 amendé] Ce tour de correction est lui-même une instance informelle du
>   Round 4 : clôture provisoire (validation LLM_B) suivie d'un changement mineur
>   (réécriture du bullet, sans changement de but/critères/interdictions) traité par
>   ajustement de la synthèse existante plutôt que par rebouclage complet — la skill
>   s'est comportée comme elle le prescrit.
> - v2.2 → v2.3 : intégration de 4 corrections issues de la revue par Gemini (juge
>   tiers) et 1 formalisation issue de ChatGPT (juge tiers), marquées `[v2.3]`, dans un
>   objectif de sobriété (pas d'usine à gaz, aucun nouveau mécanisme lourd) —
>   (1) règle de séquencement anti-court-circuit : le Round 2 doit avoir lieu avant tout
>   rebouclage au Round 4, pour que le second LLM puisse challenger l'angle subversif ;
>   (2) protection anti-ancrage du Palier 3 : le challenger génère 2-3 angles morts à
>   l'aveugle avant de lire la position de l'autre rôle ; (3) fork conditionnel : si
>   l'humain ne peut trancher immédiatement un angle subversif, le traiter comme
>   hypothèse conditionnelle et reporter l'arbitrage au Round 3 ; (4) filtre de
>   pertinence des acquis : lors d'un changement substantiel, transmettre seulement la
>   cartographie factuelle et les faits vérifiés, les arbitrages antérieurs étant
>   caducs ; (5) rôle du juge comme générateur d'angles morts : avant tout arbitrage,
>   produire au moins un angle mort indépendant des deux analyses.
...
- [v2.2] **Le biais humain est un risque aux DEUX extrémités du processus** : à l'entrée
  ...
  non convergée — voir Round 4). [v2.3] Il est aussi un risque au MILIEU (quand l'humain
  est appelé à trancher un angle subversif en cours de processus — voir le fork
  conditionnel du Round 1.5, qui évite de le faire trancher à l'aveugle). Le protocole
  protège ces trois points, mais l'humain reste le garant ultime.
...
## [v2] Calibration / dosage [v2.2 amendé, v2.3 amendé]
...
- [v2.2] **Palier 3 — Mode compressé / cadrage délégué** (décisions où un seul rôle peut
  ...
  **délégués à un seul rôle** (ex. LLM_A briefe), l'autre rôle **challengeant directement
  ...
  - [v2.3] **Protection anti-ancrage** : pour ne pas transformer le challenger (LLM_B)
    en relecteur passif (biais d'ancrage massif sur un texte déjà structuré, qui
    annulerait la défense anti-homogénéisation du Round 0), LLM_B **ne lit pas** la
    position détaillée de LLM_A immédiatement. **Avant** de la consulter, LLM_B génère
    obligatoirement **2 à 3 angles morts à l'aveugle**, à partir du seul brief/objectif
    initial (sans connaître la proposition de LLM_A). Ensuite seulement, il lit la
    position de LLM_A et procède au challenge critique. Cette micro-divergence préserve
    le moteur de divergence tout en gardant le palier compressé (2-3 angles, pas un
    Round 1 complet).
...
- [v2.3] **Séquencement anti-court-circuit** : aucun rebouclage au Round 0 (Round 4)
  n'est acté avant que le Round 2 (lecture croisée) ait eu lieu — le second LLM doit
  toujours pouvoir challenger un angle subversif avant qu'il ne déclenche une nouvelle
  itération (voir Round 1.5).
...
## [v2] Round 1.5 — Révision de l'intention et re-cartographie [v2.3 amendé]
...
[v2.3] **Règle de séquencement (anti-court-circuit)** : même si un angle
`[subvertit l'intention]` ou `[branche absente]` semble exiger un rebouclage immédiat
(Round 4), **le Round 2 (lecture croisée) doit impérativement avoir lieu avant de
relancer un Round 0**. Le second LLM doit avoir l'opportunité d'analyser l'angle
subversif : il peut révéler que l'angle repose sur un fait faux ou une prémisse erronée,
rendant le rebouclage inutile. Le rebouclage au Round 4 n'est formellement acté que si
l'angle subversif est confirmé à l'issue du Round 2 (ou validé/forké par l'humain).
...
1. **Révision de l'intention** (si tag `[subvertit l'intention]`) :
   ...
   - [v2.3] **Arbitrage ou fork conditionnel** : si l'humain peut trancher la pertinence
     de l'angle, il le fait (maintien ou révision). Si l'impact technique de l'angle est
     trop incertain pour être tranché immédiatement, **ne pas forcer un choix
     arbitraire** : créer un *fork conditionnel* dans la suite du processus (ex. « Si
     l'angle X est fondé, alors Option A ; sinon Option B »). La décision d'intention
     est alors reportée au Round 3, une fois les conséquences des deux branches
     pleinement cartographiées.
   - Si l'intention est révisée **substantiellement** (changement du but final, des
     ...
     itérative (Round 4)** : repartir du Round 0 avec la nouvelle intention — [v2.3]
     **après complétion du Round 2** (voir règle de séquencement ci-dessus).
...
## Round 2 — Lecture croisée et chasse aux contradictions [v2 amendé, v2.2 amendé, v2.3 amendé]
...
6. [v2] Vérifie aussi les angles de l'autre tagués `[subvertit l'intention]` /
   ...
   tranche** (par cohérence avec le Round 1.5). [v2.3] Si l'humain ne peut trancher
   immédiatement (impact technique trop incertain), traiter l'angle sous forme de
   **fork conditionnel** dans la synthèse (Round 3) au lieu de bloquer ou de trancher
   à l'aveugle.
7. [v2.3] **Confirmer ou infirmer les angles subversifs** : si un angle
   `[subvertit l'intention]` ou `[branche absente]` a été signalé au Round 1, le second
   LLM doit explicitement dire, à l'issue de ce Round 2, s'il le **confirme** (l'angle
   résiste à la lecture croisée) ou l'**infirme** (l'angle repose sur un fait faux ou
   une prémisse erronée). Cette confirmation conditionne le déclenchement éventuel du
   Round 4 (voir règle de séquencement du Round 1.5).

Tu ne fusionnes pas encore les plans ici. Tu exposes les contradictions et les risques.

## Round 3 — Convergence ou désaccord structuré [v2 amendé, v2.3 amendé]
...
2. Propose un plan ou une recommandation qui :
   ...
   - Garde visibles les contradictions, les hypothèses non validées et les zones
     d'ignorance.
   - [v2.3] Intègre les **forks conditionnels** éventuels (issues du Round 1.5 ou du
     Round 2) sous forme de plan à branches : « Si l'angle X est fondé, alors Option A
     (avec ses conséquences) ; sinon Option B (avec les siennes) » — la décision entre
     les branches est laissée à l'humain, en connaissance de cause.
3. Formule une section dédiée :
   ...
   - [v2.3] « Forks conditionnels ouverts » — si des angles subversifs n'ont pas pu
     être tranchés, y lister les branches conditionnelles et ce qui permettrait de les
     départager.
...
## [v2] Round 4 — Boucle itérative [v2.1 amendé, v2.2 amendé, v2.3 amendé]
...
(ou en cours de Round 1.5/3), de façon mineure ou substantielle. [v2.3] Le
déclenchement est toujours **subordonné à la règle de séquencement** (Round 2 avant
tout rebouclage — voir Round 1.5).
...
- [v2.3] **Filtre de pertinence des acquis (anti-héritage toxique)** : la nature de ce
  qui est transmis dépend du type de changement :
  - **Changement mineur** : l'intégralité de la synthèse précédente sert de contexte
    d'acquis (décisions, contraintes et angles).
  - **Changement substantiel** (modification du but, des critères ou des interdictions) :
    la synthèse transmise sert de contexte **uniquement pour la cartographie factuelle,
    les dépendances techniques et les faits vérifiés**. En revanche, les **arbitrages,
    choix de design et compromis** de la synthèse précédente sont **explicitement
    caducs** — les LLM doivent réévaluer la solution sans se sentir liés par des
    recommandations élaborées sous l'ancienne intention obsolète.
  - Pour trancher : **faits vérifiés / cartographie factuelle** = ce qui a été établi
    comme vrai ou contraint (données confirmées, dépendances techniques, contraintes
    vérifiées) ; **arbitrages / choix de design** = les décisions prises pour satisfaire
    l'ancienne intention (recommandations, options retenues, compromis). En cas de
    doute sur un élément, le traiter comme caduc (mieux vaut réévaluer à tort que
    réinjecter un arbitrage obsolète).
...
## Juge final (optionnel) [v2 amendé, v2.2 amendé, v2.3 amendé]
...
- [v2.3] **Produis au moins un angle mort indépendant** des deux analyses, avant tout
  arbitrage : le juge n'est pas seulement un auditeur/vérificateur, mais un **troisième
  cerveau**. Cette capacité (produire un angle mort inédit que les deux LLM n'ont pas
  vu) a été, dans la pratique de la skill, la source des découvertes les plus
  déterminantes — elle est ici formalisée plutôt que laissée à l'initiative du juge.
...
- [v2.1] Vérifie que la synthèse précédente a bien été transmise comme contexte à
  chaque rebouclage (anti-perte d'acquis), [v2.3] et que le filtre de pertinence des
  acquis a été appliqué (pas d'arbitrages obsolètes réinjectés lors d'un changement
  substantiel).
...
- [v2.3] Vérifie que la règle de séquencement a été respectée (Round 2 avant tout
  rebouclage) et que les forks conditionnels ouverts sont documentés.
- Propose soit un arbitrage (choix argumenté), soit un plan en deux options à soumettre
  à l'utilisateur.

## Discipline et sécurité [v2 amendé, v2.2 amendé, v2.3 amendé]
...
- [v2.1] Ne repars jamais d'une itération sans transmettre la synthèse précédente
  (anti-perte d'acquis), [v2.3] en appliquant le filtre de pertinence (pas
  d'arbitrages obsolètes réinjectés lors d'un changement substantiel).
...
- [v2.3] Ne déclenche jamais un rebouclage (Round 4) avant que le Round 2 ait eu lieu :
  le second LLM doit toujours pouvoir confirmer ou infirmer l'angle subversif avant
  qu'il ne relance une itération (anti-court-circuit).
- [v2.3] Ne force jamais l'humain à trancher à l'aveugle un angle subversif dont
  l'impact est incertain : utiliser le fork conditionnel et reporter l'arbitrage au
  Round 3, en connaissance de cause.

## Format de sortie [v2 amendé, v2.2 amendé, v2.3 amendé]
...
- [v2.3] Palier utilisé (1 rounds complets / 2 raccourci / 3 compressé) ; si Palier 3,
  les 2-3 angles morts à l'aveugle générés par le challenger avant lecture du brief.
...
### [v2] Révision / re-cartographie (Round 1.5, si applicable) [v2.3 amendé]
...
- [v2.3] Forks conditionnels ouverts (si l'humain n'a pas pu trancher immédiatement).
- Itération déclenchée (si révision substantielle) — [v2.3] après complétion du
  Round 2 (règle de séquencement).

### Lecture de l'autre LLM (Round 2) [v2.3 amendé]
...
- [v2.3] Confirmation ou infirmation des angles subversifs signalés au Round 1
  (l'angle résiste-t-il à la lecture croisée, ou repose-t-il sur un fait faux / une
  prémisse erronée ?).

### Synthèse (Round 3) [v2.3 amendé]

- Points compatibles.
- Points contradictoires (non résolus).
- Angles complémentaires.
- Recommandations.
- [v2.3] Forks conditionnels (plan à branches : si X fondé alors Option A, sinon
  Option B), le cas échéant.
- [v2] Ce qui a été révisé (intention / cartographie) et pourquoi.
...
### [v2] Itérations (Round 4, si applicable) [v2.1 amendé, v2.2 amendé, v2.3 amendé]
...
- [v2.3] Filtre de pertinence appliqué (changement substantiel → seuls les faits
  vérifiés / la cartographie factuelle transmis ; arbitrages antérieurs caducs).
...
- [v2.3] Règle de séquencement respectée (Round 2 avant rebouclage : oui/non).
...
## Conseils pour l'utilisateur humain [v2 amendé, v2.2 amendé, v2.3 amendé]
...
5. [v2] **Calibre selon l'enjeu** (trois paliers) : palier 1 (rounds complets) pour
   ...
   quand un seul rôle peut cadrer efficacement — [v2.3] en exigeant du challenger 2-3
   angles morts à l'aveugle avant de lire la proposition (protection anti-ancrage).
...
8. [v2] Si des angles `[subvertit l'intention]` / `[branche absente]` apparaissent,
   ...
   angle, **tranche**. [v2.3] Si tu ne peux pas trancher immédiatement (impact
   incertain), demande un **fork conditionnel** (l'arbitrage sera reporté au Round 3).
9. Envoie la réponse de LLM_A à LLM_B avec la consigne : « Voici la position et les
   ...
   [v2.3] Demande à chacun de **confirmer ou infirmer** explicitement les angles
   subversifs signalés au Round 1 — aucun rebouclage n'est acté avant cette étape.
10. Demande à chacun un Round 3 de synthèse : « Fais la synthèse selon le Round 3, en
    ...
    vérifier. » [v2.3] Intègre les forks conditionnels ouverts sous forme de plan à
    branches.
11. [v2] Si l'intention/les contraintes ont changé substantiellement, fais le **Round
    ...
    consolidant la synthèse précédente — [v2.3] **après complétion du Round 2** (règle
    de séquencement). [v2.1] **Transmets la synthèse consolidée précédente comme
    contexte** aux deux LLM (anti-perte d'acquis). [v2.3] Lors d'un changement
    substantiel, ne transmets que les **faits vérifiés / la cartographie factuelle**
    (les arbitrages antérieurs sont caducs). [v2.2] Demande-leur de diverger sur les
    points non résolus (indépendance affaiblie mais informée).
...
13. Optionnel : appelle un troisième LLM comme juge pour trancher ou proposer des
    options — [v2.3] en lui demandant explicitement de **produire au moins un angle
    mort indépendant** des deux analyses avant tout arbitrage.
